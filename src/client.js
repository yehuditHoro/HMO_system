...
const Client = () => {
  ...
  const [IsSick,setSick]=useState(null);
    ...
    const handleSubmit = (e) => {
      ...
      // Add new member data to your database or API here
 ...
 const addMember = () => {
  ...
  fetch('/api/members', {
    ...
    .then(data => {
      ...
      setNewMember({
        name: '',
        idNumber: '', // Assuming ID number is also a field
        address: '',
        city: '',
        birthDate: '',
        phone: '',
        mobilePhone: '',
        vaccinationDate: '', // Vaccination details
        illnessDate: '',
        recoveryDate: ''
      });
    })
    ...
    <div>
      ...
      {IsSick && (
        <div>
          ...
          <p>תאריך חיסון: {selectedMember.vaccinationDate}</p>
          <p>תאריך מחלה: {selectedMember.illnessDate}</p>
          <p>תאריך החלמה: {selectedMember.recoveryDate}</p>
        </div>
      )}
      <h2>הוספת חבר חדש</h2>
      ...
      {/* <button onClick={addMember}>הוסף חבר</button> */}
    </form>
      <button onClick={() => deleteMember(selectedMember.id)}>מחק חבר</button>
